...
fn parse_stmt(input: InType) -> ParseOutput<Stmt, InType> {
	Err(0)
...
fn parse_expr(input: InType) -> ParseOutput<Expr, InType> {
	Err(0)
...
fn peek_token(input: InType) -> ParseResult<Lexeme> {
	match input.first() {
		Some(tk) => Ok(tk.clone()),
		None => Err(2)
	}
...
fn ignore_token(input: InType) -> ParseResult<InType> {
	if input.is_empty() {
		return Err(3)
	}
	Ok(&input[1..])
